...
export function TaskDrawer({ task, members, onClose, onUpdated, onDeleted }: Props) {
  ...
  return (
    ...
    >
      <div
        className="slide-up"
        style={{ background: 'var(--bg-muted)', width: '100%', maxWidth: 520, borderRadius: '18px 18px 0 0', maxHeight: '92vh', overflowY: 'auto', boxShadow: '0 -4px 24px rgba(0,0,0,0.10)' }}
      >
        {/* Handle + close */}
        <div style={{ position: 'sticky', top: 0, background: 'var(--bg-muted)', padding: '14px 16px 10px', zIndex: 1 }}>
          <div style={{ width: 36, height: 3, background: 'var(--border)', borderRadius: 99, margin: '0 auto 14px' }} />
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <button
              onClick={onClose}
              style={{ background: 'var(--border)', border: 'none', borderRadius: 7, padding: '5px 12px', fontSize: 13, color: 'var(--text-secondary)', cursor: 'pointer' }}
            >Close</button>
          </div>
        </div>

        {/* Zone 1: Identity */}
        <div style={{ background: 'var(--bg-surface)', margin: '0 10px', borderRadius: 12, padding: '14px 14px 4px', marginBottom: 8, border: '1px solid var(--border)' }}>
          <input
            value={current.title}
            ...
            style={{ width: '100%', fontSize: 18, fontWeight: 700, border: 'none', outline: 'none', background: 'transparent', marginBottom: 10, color: 'var(--text-primary)', boxSizing: 'border-box' }}
          />
          <input
            value={current.notes}
            ...
            style={{ width: '100%', fontSize: 13, border: 'none', borderTop: '1px solid #f4f4f5', outline: 'none', background: 'transparent', padding: '10px 0', color: 'var(--text-secondary)', boxSizing: 'border-box' }}
          />
        </div>

        {/* Zone 2: Classification */}
        <div style={{ background: 'var(--bg-surface)', margin: '0 10px', borderRadius: 12, padding: '14px 14px', marginBottom: 8, border: '1px solid var(--border)' }}>
          ...
          <div style={{ height: 1, background: 'var(--bg-muted)', marginBottom: 14 }} />
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            <input
              ...
              style={{ flex: 1, fontSize: 13, border: '1px solid var(--border)', borderRadius: 8, padding: '8px 10px', outline: 'none', background: 'var(--bg-subtle)', color: 'var(--text-primary)' }}
            />
            {current.due_at && (
              <button
                onClick={() => { setCurrent({ ...current, due_at: undefined }); patch({ clear_due_at: true }) }}
                style={{ fontSize: 12, color: 'var(--text-muted)', background: 'var(--bg-muted)', border: 'none', borderRadius: 7, padding: '7px 10px', cursor: 'pointer', whiteSpace: 'nowrap' }}
              >Clear</button>
            )}
          </div>
        </div>
        ...
        <div style={{ background: 'var(--bg-surface)', margin: '0 10px', borderRadius: 12, padding: '14px 14px', marginBottom: 8, border: '1px solid var(--border)' }}>
          ...
          {feed.length === 0 && (
            <p style={{ fontSize: 13, color: 'var(--text-muted)', margin: '0 0 12px' }}>No activity yet.</p>
          )}
            ...
            {feed.map((item) =>
              item.type === 'comment' ? (
                <div key={item.data.id} style={{ display: 'flex', gap: 8 }}>
                  ...
                  <div style={{ background: 'var(--bg-muted)', borderRadius: 10, padding: '8px 10px', flex: 1 }}>
                    ...
                    <div style={{ fontSize: 13, color: 'var(--text-primary)' }}>{item.data.body}</div>
                  </div>
                </div>
              ) : (
                <div key={item.data.id} style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  ...
                  <span style={{ fontSize: 12, color: 'var(--text-secondary)', lineHeight: 1.4 }}>
                    <span style={{ fontWeight: 600, color: 'var(--text-secondary)' }}>{item.data.actor?.name ?? 'Someone'}</span>
                    {' '}{describeActivity(item.data)}
                    <span style={{ color: 'var(--text-muted)' }}> · {timeAgo(item.data.created_at)}</span>
                  </span>
                </div>
              )
            )}
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <input
              ...
              style={{ flex: 1, fontSize: 13, border: '1px solid var(--border)', borderRadius: 8, padding: '9px 10px', outline: 'none', background: 'var(--bg-subtle)' }}
            />
            <button
          ...
          {confirmDelete ? (
            <div style={{ display: 'flex', gap: 8 }}>
              <button
                ...
                style={{ flex: 1, background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 10, padding: 12, color: 'var(--text-secondary)', fontWeight: 600, fontSize: 14, cursor: 'pointer' }}
              >Cancel</button>
              <button
